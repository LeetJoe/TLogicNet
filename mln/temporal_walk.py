...
class Temporal_Walk(object):
    def __init__(self, learn_data, num_relations, transition_distr):
        """
    ...
    def transition_step(self, cur_node, cur_ts, prev_edge, start_node, step, L):
        ...
        Parameters:
            ...
            step (int): number of current step (start from 1)
        ...
        if step == 1:  # The next timestamp should be smaller than the current timestamp
            ...
            inv_edge = [
                cur_node,
                prev_edge[1] + self.num_relations if prev_edge[1] < self.num_relations else prev_edge[1] - self.num_relations,
                prev_edge[0],
                cur_ts,
            ]
